...
export async function extractRelatedAccountFromBlock (block: SubstrateBlock): Promise<string[]> {
    ...
    if (block.block.extrinsics.length!==0){
        for (const extrinsic of block.block.extrinsics){
            const extAccounts = flatten(nestExtractCalls(extrinsic.method))
            accounts.concat(flatten(nestExtractCalls(extrinsic.method)))
        }
    }
...
export function nestExtractCalls(call: CallBase<AnyTuple>){
    ...
    for (const [key, arg] of Object.entries(call.meta.args.toArray())){
        ...
        if(call.section == 'utility' && call.method == 'batchAll' ){
            ...
            return accounts.concat(
                flatten(
                    calls.map((call) => nestExtractCalls(call)),
                ),
            );
        }
...
export function extractRelatedAccountFromEvent (event: SubstrateEvent): string[]{
    const accounts: string[] = [];
    if(event.extrinsic.extrinsic.isSigned && event.extrinsic.extrinsic.signer){
        accounts.push(event.extrinsic.extrinsic.signer.toString());
    }
    for (const [key, typeDef] of Object.entries(event.event.data.typeDef)){
        if(typeDef.type === `AccountId`){
            const index = Number(key);
